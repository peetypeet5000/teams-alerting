 ...
 */
async function doPost(message: any, url: string): Promise<any> {
  const postMessage = {
    method: 'POST',
    url: `${url}`,
    data: message,
 ...
 */
export function setup(appName: string, channels: Channel[]): void {
  if (appName.length < 1) {
